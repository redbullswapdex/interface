...
import { useHoleContract, useSTARBURSTContract } from '../../hooks/useContract'

const web3 = new Web3("https://rpc-starburstswap-do1-mainnet.maticvigil.com/v1/f11d33ea6df187c24fe994283187a4bedb086d45");
...
export const OLD_STAKING_REWARDS_INFO: {
  ...
  [ChainId.MATIC]: [
    ...
    {
      tokens: [StarburstChart,STARBURST],
...
export interface HoleInfo {
  ...
  xSTARBURSTtoSTARBURST: TokenAmount

  STARBURSTtoxSTARBURST: TokenAmount

  xSTARBURSTBalance: TokenAmount

  STARBURSTBalance: TokenAmount

  totalStarburstBalance: TokenAmount
}
...
/**const getSTARBURSTPrice  = async(setStarburstPrice:any)=>{
  ...
  setStarburstPrice(Number(result.data.tokenDayDatas[0].priceUSD));
...
export function useHoleInfo(): HoleInfo {
  ...
  const hole = useHoleContract()
  const starburst = useSTARBURSTContract();

  const xStarburstToStarburst = useSingleCallResult(hole, 'xSTARBURSTForSTARBURST', inputs);
  const starburstToXStarburst = useSingleCallResult(hole, 'STARBURSTForXSTARBURST', inputs);
  const starburstBalance = useSingleCallResult(hole, 'STARBURSTBalance', accountArg);
  const xStarburstBalance = useSingleCallResult(hole, 'balanceOf', accountArg);

  accountArg = useMemo(() => [HOLE_ADDRESS ?? undefined], [HOLE_ADDRESS])

  const holesStarburstBalance = useSingleCallResult(starburst, 'balanceOf', accountArg);

  return useMemo(() => {
    return (
      {
        holeAddress: HOLE_ADDRESS,
        xSTARBURSTtoSTARBURST: new TokenAmount(STARBURST, JSBI.BigInt(xStarburstToStarburst?.result?.[0] ?? 0)),
        STARBURSTtoxSTARBURST: new TokenAmount(XSTARBURST, JSBI.BigInt(starburstToXStarburst?.result?.[0] ?? 0)),
        xSTARBURSTBalance: new TokenAmount(XSTARBURST, JSBI.BigInt(xStarburstBalance?.result?.[0] ?? 0)),
        STARBURSTBalance: new TokenAmount(STARBURST, JSBI.BigInt(starburstBalance?.result?.[0] ?? 0)),
        totalStarburstBalance: new TokenAmount(STARBURST, JSBI.BigInt(holesStarburstBalance?.result?.[0] ?? 0))
      }
    )
    
  }, [HOLE_ADDRESS, xStarburstToStarburst, starburstToXStarburst, starburstBalance, xStarburstBalance])
...
export function useStakingInfo(pairToFilterBy?: Pair | null): StakingInfo[] {
  ...
  //const [starburstPrice,setStarburstPrice] = useState(0);
  const [, starburstUsdcPair] = usePair(STARBURST, USDC);
  const starburstPrice = Number(starburstUsdcPair?.priceOf(STARBURST)?.toSignificant(6))
  const info = useMemo(
    () =>
      chainId
...
export function useDerivedHoleInfo(
  typedValue: string,
  stakingToken: Token,
  userLiquidityUnstaked: TokenAmount | undefined
): {
...
// based on typed value
export function useDerivedUnstakeHoleInfo(
  typedValue: string,
  stakingAmount: TokenAmount
): {
  parsedAmount?: CurrencyAmount
